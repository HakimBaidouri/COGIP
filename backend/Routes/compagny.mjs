  ...
  connection.query(checkCompanyQuery,[tva],(err, result)=>{
    ...
    if(result.length>0) {
      return res.status(409).json ({
        error : "Conflict, the tva number has been already attribute to another company"
      })
    }
    
    //if the company doesn't exist insert the company
    const insertCompanyQuery = 'INSERT INTO companies (name, type_id, country, tva) VALUES (?,?,?,?)';
    connection.query(insertCompanyQuery, [name, type_id, country, tva], (err, result) => {
      if (err) {
        return res.status(500).json({
