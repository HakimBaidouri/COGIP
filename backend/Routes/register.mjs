...
router.post('/', (req, res) => {
  ...
  connection.query(checkEmailQuery, email, (err, result)=>{
    ...
    try {
      connection.query('INSERT INTO users (first_name, role_id, last_name, email, password, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',[first_name, role_id, last_name, email, password, nowTime, nowTime]);
