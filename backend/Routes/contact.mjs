...
//get all contacts
router.get('/', (req, res) => {
  const selectAllContactsQuery = 'SELECT * FROM contacts';
...
router.post('/', (req, res) => {
  ...
  connection.query(checkEmailRequest,[email],(err, result)=>{
    ...
    const insertContactQuery = 'INSERT INTO contacts (name, company_id, email, phone) VALUES (?,?,?,?)';
    connection.query(insertContactQuery, [name, company_id, email, phone], (err, result) => {
      if (err) {
        return res.status(500).json({
...
router.put('/:id', (req, res) => {
  ...
  const selectEmailQuery = 'SELECT email FROM contacts WHERE email = ? and id != ?';
